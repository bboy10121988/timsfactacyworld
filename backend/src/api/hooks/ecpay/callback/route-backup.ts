...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      // 手動過濾包含目標 MerchantTradeNo 的購物車
      const targetCart = carts?.find((cart: any) => {
        if (!cart.metadata) return false
        
        // 檢查所有可能的 metadata key
        const possibleKeys = [
          'ecpay_trade_no',
        ...
        return possibleKeys.some(key => 
          cart.metadata[key] === MerchantTradeNo
        )
      })

      if (!targetCart) {
        console.log(`❌ No cart found for MerchantTradeNo: ${MerchantTradeNo}`)
        console.log(`📊 Checked ${carts?.length || 0} carts`)
        
        // 列出所有購物車的 metadata 以便調試
        if (carts && carts.length > 0) {
          console.log('🔍 Available carts metadata:')
          carts.slice(0, 5).forEach((cart: any, index: number) => {
            console.log(`   Cart ${index + 1}: ${cart.id} - metadata:`, cart.metadata)
          })
        }
