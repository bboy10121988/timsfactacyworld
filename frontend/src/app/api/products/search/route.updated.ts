...
export async function GET(request: NextRequest) {
  ...
  const format = searchParams.get('format') // 新增格式參數: 'full' 或 'simple'
  ...
  try {
    ...
    const productsResponse = await sdk.client.fetch<ProductsResponse>(`/store/products`, {
      ...
      query: {
        ...
        fields: "handle,id,title,thumbnail,variants,options,variants.title,variants.prices.amount,variants.prices.original_amount,description",
      },
      headers: {
        "x-publishable-api-key": process.env.NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY!,
      }
        ...
        return {
          ...
          variants: product.variants?.map(variant => 
            `${variant.title} - ${variant.prices?.[0]?.amount ? 
              `NT$${variant.prices[0].amount}` : 'NT$0'}${
              variant.prices?.[0]?.original_amount ? 
                ` (原價 NT$${variant.prices[0].original_amount})` : ''
            }`
          ),
          price: {
